...
EmberAfStatus emberAfClusterSpecificCommandParse(EmberAfClusterCommand *cmd)
{
  ...
  if (cmd->direction == (uint8_t)ZCL_DIRECTION_SERVER_TO_CLIENT
    ...
    case ZCL_SHADE_CONFIG_CLUSTER_ID:
      result = status(false, true, cmd->mfgSpecific);
      break;
    case ZCL_VC_MFG_SYS_CONFIG_CLUSTER_ID:
      result = status(false, true, cmd->mfgSpecific);
      break;
    case ZCL_VC_MFG_EXTENSION_CLUSTER_ID:
      result = emberAfVcMfgExtensionClusterServerCommandParse(cmd);
      break;
    default:
      // Unrecognized cluster ID, error status will apply.
      break;
...
EmberAfStatus emberAfBasicClusterServerCommandParse(EmberAfClusterCommand *cmd)
{
  ...
  if (!cmd->mfgSpecific) {
    ...
    case ZCL_TUYA_EXT_RESET_COMMAND_ID:
      {
        // Command is fixed length: 0
        wasHandled = emberAfBasicClusterTuya_ext_resetCallback();
        break;
      }
    default:
      {
...
// Cluster: Vancount Devices Extension Cluster, server
EmberAfStatus emberAfVcMfgExtensionClusterServerCommandParse(EmberAfClusterCommand *cmd)
{
  bool wasHandled = false;
  if (cmd->mfgSpecific) {
    if (cmd->mfgCode == 0x1254 && cmd->commandId == ZCL_TRANSPARENT_REQUEST_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t* data;  // Ver.: always
      // Command is fixed length: 0
      data = cmd->buffer + payloadOffset;
      wasHandled = emberAfVcMfgExtensionClusterTransparentRequestCallback(data);
    } else if (cmd->mfgCode == 0x1254 && cmd->commandId == ZCL_ZIGBEE_EXTEND_REQ_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t* data;  // Ver.: always
      // Command is fixed length: 0
      data = cmd->buffer + payloadOffset;
      wasHandled = emberAfVcMfgExtensionClusterZigbeeExtendReqCallback(data);
    }
  }
  return status(wasHandled, true, cmd->mfgSpecific);
}
