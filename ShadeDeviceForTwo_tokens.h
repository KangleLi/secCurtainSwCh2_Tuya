...
// Creator for attribute: LedState, singleton.
#define CREATOR_ATTRIBUTE_MFG_INDICATOR_STATE_SINGLETON 0xB005
#define NVM3KEY_ATTRIBUTE_MFG_INDICATOR_STATE_SINGLETON ( NVM3KEY_DOMAIN_ZIGBEE | 0xB005 )
// Creator for attribute: TimeScale, singleton.
#define CREATOR_ATTRIBUTE_MFG_TIME_SCALE_SINGLETON 0xB006
#define NVM3KEY_ATTRIBUTE_MFG_TIME_SCALE_SINGLETON ( NVM3KEY_DOMAIN_ZIGBEE | 0xB006 )
// Creator for attribute: on/off, endpoint: 2
#define CREATOR_ON_OFF_2 0xB007
#define NVM3KEY_ON_OFF_2 ( NVM3KEY_DOMAIN_ZIGBEE | 0xB007 )
// Creator for attribute: start up on off, endpoint: 2
#define CREATOR_START_UP_ON_OFF_2 0xB008
#define NVM3KEY_START_UP_ON_OFF_2 ( NVM3KEY_DOMAIN_ZIGBEE | 0xB008 )
// Creator for attribute: current level, endpoint: 2
#define CREATOR_CURRENT_LEVEL_2 0xB009
#define NVM3KEY_CURRENT_LEVEL_2 ( NVM3KEY_DOMAIN_ZIGBEE | 0xB009 )
// Creator for attribute: start up current level, endpoint: 2
#define CREATOR_START_UP_CURRENT_LEVEL_2 0xB00A
#define NVM3KEY_START_UP_CURRENT_LEVEL_2 ( NVM3KEY_DOMAIN_ZIGBEE | 0xB00A )
// Creator for attribute: closed limit, endpoint: 2
#define CREATOR_SHADE_CONFIG_CLOSED_LIMIT_2 0xB00B
#define NVM3KEY_SHADE_CONFIG_CLOSED_LIMIT_2 ( NVM3KEY_DOMAIN_ZIGBEE | 0xB00B )
...
DEFINE_BASIC_TOKEN(SHADE_CONFIG_CLOSED_LIMIT_1, tokType_shade_config_closed_limit, 255)
DEFINE_BASIC_TOKEN(ATTRIBUTE_MFG_INDICATOR_STATE_SINGLETON, tokType_attribute_mfg_indicator_state, 0)
DEFINE_BASIC_TOKEN(ATTRIBUTE_MFG_TIME_SCALE_SINGLETON, tokType_attribute_mfg_time_scale, 0)
...
DEFINE_BASIC_TOKEN(SHADE_CONFIG_CLOSED_LIMIT_2, tokType_shade_config_closed_limit, 255)
#endif // DEFINETOKENS
...
#define GENERATED_TOKEN_LOADER(endpoint) do {\
  ...
  uint8_t epNetwork; \
  halCommonGetToken((tokType_attribute_mfg_indicator_state *)ptr, TOKEN_ATTRIBUTE_MFG_INDICATOR_STATE_SINGLETON); \
  ...
  halCommonGetToken((tokType_attribute_mfg_time_scale *)ptr, TOKEN_ATTRIBUTE_MFG_TIME_SCALE_SINGLETON); \
  ...
  if((endpoint) == 2 || ((endpoint) == EMBER_BROADCAST_ENDPOINT && epNetwork == curNetwork)) { \
    ...
    halCommonGetToken((tokType_shade_config_closed_limit *)ptr, TOKEN_SHADE_CONFIG_CLOSED_LIMIT_2); \
    emberAfWriteServerAttribute(2, ZCL_SHADE_CONFIG_CLUSTER_ID, ZCL_SHADE_CONFIG_CLOSED_LIMIT_ATTRIBUTE_ID, (uint8_t*)ptr, ZCL_INT16U_ATTRIBUTE_TYPE); \
  } \
} while(false)
...
#define GENERATED_TOKEN_SAVER do {\
  ...
  if ( clusterId == 0xFC56 ) { \
    if ( metadata->attributeId == 0x0000 && 0x1254 == getManufacturerCode( (EmberAfManufacturerCodeEntry *) attributeManufacturerCodes, attributeManufacturerCodeCount, (metadata - generatedAttributes)) &&!emberAfAttributeIsClient(metadata) ) \
      halCommonSetToken(TOKEN_ATTRIBUTE_MFG_INDICATOR_STATE_SINGLETON, data); \
    if ( metadata->attributeId == 0x0004 && 0x1254 == getManufacturerCode( (EmberAfManufacturerCodeEntry *) attributeManufacturerCodes, attributeManufacturerCodeCount, (metadata - generatedAttributes)) &&!emberAfAttributeIsClient(metadata) ) \
      halCommonSetToken(TOKEN_ATTRIBUTE_MFG_TIME_SCALE_SINGLETON, data); \
  }\
  if ( endpoint == 1 ) { \
    if ( clusterId == 0x06 ) { \
    ...
    if ( clusterId == 0x06 ) { \
      ...
      if ( metadata->attributeId == 0x0010 && 0x0000 == getManufacturerCode( (EmberAfManufacturerCodeEntry *) attributeManufacturerCodes, attributeManufacturerCodeCount, (metadata - generatedAttributes)) &&!emberAfAttributeIsClient(metadata) ) \
        halCommonSetToken(TOKEN_SHADE_CONFIG_CLOSED_LIMIT_2, data); \
    } \
  } \
} while(false)
