...
extern void vcShadeOperateFromLevelAbort(uint8_t endpoint);
#define updateCoupledColorTemp(endpoint) vcShadeOperateFromLevel((endpoint))
                        ...
                        uint8_t optionOverride)
{
  ...
  deactivate(endpoint);
  writeRemainingTime(endpoint, 0);
  vcShadeOperateFromLevelAbort(endpoint);
  status = EMBER_ZCL_STATUS_SUCCESS;
  send_default_response:
  emberAfSendImmediateDefaultResponse(status);
}
  ...
  if (newValue) {
    ...
    if (status != EMBER_ZCL_STATUS_SUCCESS) {
      ...
      return;
		...
		// If newValue is ZCL_ON_COMMAND_ID...
		// "Set CurrentLevel to minimum level allowed for the device."
		currentOnLevel = minimumLevelAllowedForTheDevice;
		status = emberAfWriteServerAttribute(endpoint,
		ZCL_LEVEL_CONTROL_CLUSTER_ID,
		ZCL_CURRENT_LEVEL_ATTRIBUTE_ID,
				(uint8_t *) &currentOnLevel,
