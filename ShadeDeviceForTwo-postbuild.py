...
def S37():
    ...
    if ("gcc" == "iar" or "gcc" == ""):
        ...
        GCC_TOOLCHAIN_PATH = shortPath(r"file:/J:/SiliconLabs/SimplicityStudio/v4/developer/toolchains/gnu_arm/7.2_2017q4/".replace(
            "file:", "").replace("%20", " "))
...
def EBL():
    ...
    print " "
    EBL_COMMAND = START_FLAGS + shortPath(r"J:\SiliconLabs\SimplicityStudio\v4\developer\adapter_packs\commander\commander.exe").replace(
...
def GBL():
    ...
    GBL_COMMAND = START_FLAGS + shortPath(r"J:\SiliconLabs\SimplicityStudio\v4\developer\adapter_packs\commander\commander.exe").replace(
...
def OTA():
    if not '"%PROJECT_DIR%\..\\..\\..\\protocol\\zigbee\\tool\\image-builder\\image-builder-windows.exe" --create "%TARGET_BPATH%.ota" --version 205 --manuf-id 0x1254 --image-type 0x0302 --tag-id 0x0000 --tag-file "%TARGET_BPATH%.gbl" --string "EBL ShadeDeviceForTwo"':
        sys.exit(0)
    print " "
    ...
    IMAGE_BUILDER = '"%PROJECT_DIR%\..\\..\\..\\protocol\\zigbee\\tool\\image-builder\\image-builder-windows.exe" --create "%TARGET_BPATH%.ota" --version 205 --manuf-id 0x1254 --image-type 0x0302 --tag-id 0x0000 --tag-file "%TARGET_BPATH%.gbl" --string "EBL ShadeDeviceForTwo"'.replace("\t", "\\t")
