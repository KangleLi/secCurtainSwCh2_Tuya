...
#define BUTTON4_IRQ    ((uint8_t)0x10)

#define BUTTON_FOR_JOIN_NET     (4)
#define BUTTON_IRQ_FOR_JOIN_NET BUTTON4_IRQ
static bool buttonPress(uint8_t button, uint8_t state);
static uint32_t  buttonPressDurationMs[HAL_BUTTON_COUNT] = {0};
...
void emberAfPluginButtonJoiningButton1EventHandler(void)
{
	...
	if (buttonPressFlag & BUTTON0_IRQ) {
		buttonPressDurationMs[0] = elapsedTimeInt32u(buttonPressDurationMs[0],
	...
	if (buttonPressFlag & BUTTON1_IRQ) {
		buttonPressDurationMs[1] = elapsedTimeInt32u(buttonPressDurationMs[1],
				halCommonGetInt32uMillisecondTick());
	...
	if (buttonPressFlag & BUTTON2_IRQ) {
		buttonPressDurationMs[2] = elapsedTimeInt32u(buttonPressDurationMs[2],
				halCommonGetInt32uMillisecondTick());
		if (buttonPressDurationMs[2] < BUTTON_SHORT_PRESSED_MS) {
			buttonPressFlag &= ~BUTTON2_IRQ;
			buttonReleaseFlag &= ~BUTTON2_IRQ;
			state = 0;
		}
		if (buttonPressDurationMs[2] >= BUTTON_SHORT_PRESSED_MS) {
			if (buttonReleaseFlag & BUTTON2_IRQ) {
				state = 2;
				buttonPressFlag &= ~BUTTON2_IRQ;
				buttonReleaseFlag &= ~BUTTON2_IRQ;
			} else {
		...
		vcAfPluginButtonJoiningButtonEventCallback(2, // button 2 is pressed
				state);
	} else {
		buttonReleaseFlag &= ~BUTTON2_IRQ;
	}
	...
	if (buttonPressFlag & BUTTON3_IRQ) {
		buttonPressDurationMs[3] = elapsedTimeInt32u(buttonPressDurationMs[3],
				halCommonGetInt32uMillisecondTick());
		if (buttonPressDurationMs[3] < BUTTON_SHORT_PRESSED_MS) {
			buttonPressFlag &= ~BUTTON3_IRQ;
			buttonReleaseFlag &= ~BUTTON3_IRQ;
			state = 0;
		}
		if (buttonPressDurationMs[3] >= BUTTON_SHORT_PRESSED_MS) {
			if (buttonReleaseFlag & BUTTON3_IRQ) {
				state = 2;
				buttonPressFlag &= ~BUTTON3_IRQ;
				buttonReleaseFlag &= ~BUTTON3_IRQ;
			} else {
		...
		vcAfPluginButtonJoiningButtonEventCallback(3, // button 1 is pressed
				state);
	} else {
		buttonReleaseFlag &= ~BUTTON3_IRQ;
	}
...
static const vcButtonPin[HAL_BUTTON_
COUNT] = {BUTTON0};
...
#elif HAL_BUTTON_COUNT == 5
static const vcButtonPin[HAL_BUTTON_COUNT] = {BUTTON0, BUTTON1, BUTTON2, BUTTON3, BUTTON4};
#endif
void customButtonTimeoutEventHandler(void)
{
	...
	if ((keyAbortCount == 3 || keyAbortCount == 10) &&
			(buttonPressFlag & BUTTON_IRQ_FOR_JOIN_NET)) {
...
void emberAfHalButtonIsrCallback(uint8_t button, uint8_t state)
{
	...
	case BUTTON2:
		if (state == BUTTON_PRESSED) {
			buttonPressDurationMs[2] = halCommonGetInt32uMillisecondTick();
			buttonPressFlag |= BUTTON2_IRQ;
			vcSetKeyState(2);
			emberEventControlSetDelayMS(buttonEvent1, 500);
		} else {
	...
	case BUTTON3:
		if (state == BUTTON_PRESSED) {
			buttonPressDurationMs[3] = halCommonGetInt32uMillisecondTick();
			buttonPressFlag |= BUTTON3_IRQ;
			vcSetKeyState(3);
			emberEventControlSetDelayMS(buttonEvent1, 500);
		} else {
		...
		break;
#endif
#if HAL_BUTTON_COUNT >= 5
	case BUTTON4:
		if (state == BUTTON_PRESSED) {
			buttonPressDurationMs[BUTTON_FOR_JOIN_NET] =
					halCommonGetInt32uMillisecondTick();
			buttonPressFlag |= BUTTON4_IRQ;
			vcSetKeyState(4);
		} else {
			buttonPressFlag &= ~BUTTON4_IRQ;
			//buttonReleaseFlag |= BUTTON4_IRQ;
		}
		break;
#endif
	default:
